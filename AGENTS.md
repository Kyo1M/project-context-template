...
- **コミットの区切りは skill 1 回の成果物**（`meeting-minutes` なら議事録・台帳・決定・`llms.txt`・wiki をまとめて 1 コミット）。同じセッションで `task-issue` skill まで続けるとき、まだ push していなければ台帳の書き戻しを同じブランチ・同じ PR に含めてよい
